...
import db.dbcore.transaction.RsqlThreadTransactionBean;
...
import static db.dbcore.transaction.TransactionManager.isTransaction;
...
public class RDSManager {
    ...
    public static Connection getLocalConnection(final String spaceName) {
        ...
        if (isTransaction(spaceName)){
            return RsqlThreadTransactionBean.RsqlThreadManager.getConnection(spaceName);
        }else {
            try {
